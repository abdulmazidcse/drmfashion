...
export async function generateMetadata() {
  ...
  return {
    title: `Women | ${storeName} — Women's Fashion`,
    description: `Shop women's clothing at ${storeName}. Tops, dresses, leggings, jeans and activewear, graded in short, regular and long.`,
  };
}
...
export default async function WomenPage() {
  ...
  // render four links.
  const [categories, products, womenCategory] = await Promise.all([
  ...
  const collections = womenCategory?.children || [];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative w-full h-[70vh] min-h-[500px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-8 pb-16 w-full">
          ...
          <p className="text-faint text-sm sm:text-base font-light max-w-md mb-8">
            Premium womenswear, cut in-house. Every inseam, every silhouette — perfectly proportioned.
          </p>
          <Link
            href="/shop"
      ...
      <div className="max-w-[1600px] mx-auto px-8 py-5 w-full">
        <div className="text-[10px] text-faint font-bold uppercase tracking-[0.14em] flex items-center gap-2">
          ...
          <span className="text-soft">Women</span>
        </div>
      </div>
      ...
      <section className="max-w-[1600px] mx-auto px-8 pb-20 w-full">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight">Shop by Category</h2>
          </div>
          <Link href="/shop" className="hidden sm:flex items-center gap-2 text-[11px] font-bold tracking-[0.14em] uppercase text-soft hover:text-brand-700 transition-colors">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {collections.map((col) => (
            <Link
              key={col.id}
              href={`/category/womens-clothing?sub=${col.slug}`}
      ...
      <section className="bg-brand-ink text-white py-16 px-8 text-center mb-20">
        <p className="text-[10px] font-bold tracking-[0.4em] uppercase text-faint mb-3">Limited Time</p>
        ...
        <p className="text-faint text-sm mb-8 max-w-md mx-auto">
          Shop our full range of tops, now at 25% off. Made well, priced fairly.
        </p>
        <Link
          href="/shop"
      ...
      <section className="max-w-[1600px] mx-auto px-8 pb-24 w-full">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-faint mb-1">Curated for You</p>
            <h2 className="text-2xl sm:text-3xl font-extrabold uppercase tracking-tight">Featured Pieces</h2>
          </div>
          <Link href="/shop" className="hidden sm:flex items-center gap-2 text-[11px] font-bold tracking-[0.14em] uppercase text-soft hover:text-brand-700 transition-colors">
            View All <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        ...
        {allProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {allProducts.map((product, idx) => (
              ...
              <ProductCard key={product.id} product={product} idPrefix="women" priority={idx < 4} />
            ))}
          </div>
        )}
