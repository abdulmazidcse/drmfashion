...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: "/pages/contact-support" },
  }
}
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans antialiased text-zinc-950">
      ...
      <section className="w-full bg-zinc-950 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 md:py-20">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-7">
              ...
              {QUICK_LINKS.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="group bg-white border border-zinc-200 rounded-xl p-5 hover:border-zinc-950 hover:-translate-y-0.5 transition-all"
                  >
                    <Icon className="w-5 h-5 text-zinc-950 mb-4" />
                    <h3 className="text-xs font-black uppercase tracking-wide text-zinc-950 flex items-center gap-1">
                      {link.title}
                      <ChevronRight className="w-3.5 h-3.5 text-zinc-300 group-hover:text-zinc-950 group-hover:translate-x-0.5 transition-all" />
                    </h3>
                    <p className="text-[11px] text-zinc-500 mt-1.5 leading-relaxed">{link.desc}</p>
                  </Link>
                )
              })}
            </div>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12 md:py-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
            ...
            <div className="lg:col-span-7 xl:col-span-8">
              <div className="border border-zinc-200 rounded-2xl p-6 md:p-10">
                <div className="flex items-start gap-3 mb-1">
                  <MessageSquare className="w-5 h-5 text-zinc-950 mt-0.5 shrink-0" />
                  <div>
                    <h2 className="text-lg font-black uppercase tracking-tight text-zinc-950">Send us a message</h2>
                    <p className="text-sm text-zinc-500 mt-1">
                      Fields marked <span className="text-zinc-700 font-semibold">*</span> are required. Adding your
                      order number gets you a faster answer.
                    </p>
                  </div>
                </div>

                <div className="h-px bg-zinc-100 my-7" />
            ...
            <aside className="lg:col-span-5 xl:col-span-4 space-y-4">
                ...
                {channels.map((channel) => {
                  ...
                  const inner = (
                    <div className="p-5 flex items-start gap-4">
                      <div className="w-9 h-9 shrink-0 border border-zinc-200 rounded-lg flex items-center justify-center">
                        <Icon className="w-4 h-4 text-zinc-950" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1">{channel.label}</p>
                        <p className="text-sm font-semibold text-zinc-900 break-words leading-snug">{channel.value}</p>
                        <p className="text-[11px] text-zinc-400 mt-1.5 leading-relaxed">{channel.note}</p>
                      </div>
                    </div>
                  )
                  return channel.href ? (
              ...
              <div className="bg-zinc-950 text-white rounded-2xl p-6">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest mb-2">Already ordered?</h3>
                ...
                <Link
                  href="/track-order"
                  className="inline-flex items-center gap-2 mt-5 px-5 py-3 bg-white text-zinc-950 text-[10px] font-bold uppercase tracking-widest rounded-lg hover:bg-zinc-200 transition"
                >
        ...
        <section className="border-t border-zinc-100 bg-zinc-50/60">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 py-14 md:py-20">
            ...
            <div className="space-y-3">
              {FAQS.map((faq) => (
                <details key={faq.q} className="group bg-white border border-zinc-200 rounded-xl overflow-hidden">
                  ...
                  <p className="px-5 pb-5 -mt-1 text-[13px] text-zinc-500 leading-relaxed">{faq.a}</p>
                </details>
              ))}
            </div>
            ...
            <p className="text-center text-xs text-zinc-400 mt-8">
              More answers in the{" "}
              <Link href="/pages/help-center" className="font-bold text-zinc-700 underline underline-offset-2 hover:text-zinc-950">
                Help Center
              </Link>
              .
            </p>
