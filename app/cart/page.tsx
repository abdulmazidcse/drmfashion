...
export default function CartPage() {
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        if (data) {
          ...
          const flatRate = Number(data.shipping_flat_rate || 60);
          const freeThreshold = Number(data.shipping_free_threshold || 1000);
  ...
  const effectiveShippingFee = shippingEnabled 
    ? (shippingFreeThreshold > 0 && (subtotal - discount) >= shippingFreeThreshold ? 0 : shippingFee) 
    : 0;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-cream text-foreground font-sans antialiased">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10 w-full flex-1">
        ...
        {/* BREADCRUMB */}
        <div className="flex items-center gap-2 text-[10px] text-faint font-bold uppercase tracking-[0.14em] mb-8">
          <Link href="/" className="hover:text-soft transition-colors">Home</Link>
          <ChevronRight className="w-3 h-3" />
          <Link href="/shop" className="hover:text-soft transition-colors">Shop</Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-soft">Shopping Cart</span>
        </div>

        <h1 className="text-3xl sm:text-4xl font-extrabold uppercase tracking-tight mb-2">
          Shopping Cart
        </h1>
        <p className="text-faint text-sm font-light mb-10">
        ...
        {items.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-24 h-24 rounded-full bg-cream flex items-center justify-center mb-8 ring-8 ring-brand-50">
              <ShoppingBag className="w-10 h-10 text-faint" />
            </div>
            <h2 className="text-xl font-bold uppercase tracking-wide mb-3">Your bag is empty</h2>
            <p className="text-faint text-sm font-light mb-8 max-w-xs">
              Add some premium pieces from our curated collection to get started.
            </p>
            <Link
              href="/shop"
              className="bg-brand-600 rounded-full text-white px-10 py-4 text-xs font-bold tracking-[0.14em] uppercase hover:bg-brand-700 transition-colors inline-flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {/* CART ITEMS */}
            <div className="lg:col-span-7 xl:col-span-8 space-y-0 divide-y divide-line">

              {/* Column headers */}
              <div className="hidden sm:grid grid-cols-12 gap-4 pb-3 text-[11px] font-extrabold uppercase tracking-[0.14em] text-faint">
              ...
              {items.map((item) => {
                ...
                const uniqueLengths = product
                  ? (Array.from(new Set(product.variants.filter((v: any) => v.length).map((v: any) => v.length))).filter(Boolean) as string[])
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-12 sm:col-span-6 flex gap-4 items-center">
                      ...
                      <div className="w-16 sm:w-20 shrink-0">
                        <Link href={`/product/${item.slug}`}>
                          <div className="relative aspect-square bg-cream overflow-hidden border border-line hover:border-brand-300 transition-colors">
                            <Image
                              src={item.thumbnail}
                              alt={item.title}
                              fill
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <Link
                          ...
                          className="text-xs sm:text-sm font-bold uppercase tracking-wide text-foreground hover:text-soft transition-colors line-clamp-1"
                        >
                          {item.title}
                        </Link>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                          {item.color && (
                            <div className="flex items-center gap-1.5">
                              <span className="text-[10px] text-faint font-bold uppercase tracking-wider">Color:</span>
                              <select
                                value={item.color}
                                onChange={(e) => handleUpdateVariant(item.id, e.target.value, item.size, item.length)}
                                className="text-[10px] font-extrabold text-foreground uppercase bg-transparent outline-none cursor-pointer border-b border-dashed border-line pb-0 hover:border-brand-400 transition-colors focus:border-aqua-400"
                              >
                              {uniqueColors.map((color) => (
                                <option key={color} value={color} className="text-foreground bg-white">{color}</option>
                              ))}
                            </select>
                          </div>
                        )}
                        {item.length && item.length !== "" && (
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-faint font-bold uppercase tracking-wider">Length:</span>
                            <select
                              value={item.length}
                              onChange={(e) => handleUpdateVariant(item.id, item.color, item.size, e.target.value)}
                              className="text-[10px] font-extrabold text-foreground uppercase bg-transparent outline-none cursor-pointer border-b border-dashed border-line pb-0.5 hover:border-brand-400 transition-colors focus:border-aqua-400"
                            >
                              {uniqueLengths.map((len) => (
                                <option key={len} value={len} className="text-foreground bg-white">{len}</option>
                              ))}
                            </select>
                          </div>
                        )}
                      ...
                      {item.custom && (
                        <div className="mt-2 border-l-2 border-brand-600 pl-2.5">
                          <p className="text-[10px] font-extrabold uppercase tracking-wider text-foreground">
                            Made to measure · {item.custom.templateName}
                            {item.custom.fee > 0 && (
                              <span className="ml-1.5 font-bold text-soft">
                                +{formatPrice(item.custom.fee)}
                              </span>
                            )}
                          </p>
                          <p className="mt-0.5 text-[10px] leading-relaxed text-soft">
                    ...
                    <div className="hidden sm:flex col-span-2 items-start justify-center pt-1">
                      <div className="relative inline-block bg-white">
                        <select
                          ...
                          className="text-xs font-extrabold text-foreground border border-line pl-4 pr-9 py-2 uppercase tracking-[0.14em] bg-white cursor-pointer hover:border-brand-300 focus:outline-none focus:border-aqua-400 rounded-sg appearance-none select-none"
                          style={{ minWidth: "72px" }}
                        >
                        ...
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-soft">
                    ...
                    <div className="hidden sm:flex col-span-2 items-start justify-center pt-0.5">
                      <div className="flex items-center border border-line bg-white">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center text-soft hover:text-brand-700 hover:bg-cream transition-colors"
                    ...
                    <div className="col-span-12 sm:hidden flex items-center justify-between mt-3 pt-3 border-t border-line">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-faint font-bold uppercase tracking-wider">Size:</span>
                        <select
                          value={item.size}
                          onChange={(e) => handleUpdateVariant(item.id, item.color, e.target.value, item.length)}
                          className="text-[10px] font-extrabold text-foreground uppercase bg-transparent outline-none cursor-pointer border-b border-dashed border-line pb-0.5 hover:border-brand-400 transition-colors focus:border-aqua-400"
                        >
                          {uniqueSizes.map((size) => (
                            <option key={size} value={size} className="text-foreground bg-white">{size}</option>
                          ))}
                        </select>
                      </div>
                      <div className="flex items-center border border-line bg-white">
                        <button onClick={() => handleQtyChange(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-soft hover:text-brand-700 transition-colors">
                          <Minus className="w-3 h-3" />
                        </button>
              ...
              <div className="pt-6">
                <Link
                  href="/shop"
                  className="inline-flex items-center gap-2 text-[12.5px] font-bold uppercase tracking-[0.12em] text-soft hover:text-brand-700 transition-colors"
                >
                  <ArrowLeft className="w-4 h-4" /> Continue Shopping
                </Link>
              </div>
            ...
            <div className="lg:col-span-5 xl:col-span-4">
              <div className="bg-white border border-line p-5 sm:p-6 sticky top-24 space-y-6">
                <h2 className="text-sm font-extrabold uppercase tracking-[0.14em] text-foreground">Order Summary</h2>

                {/* Promo Code */}
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-[0.14em] text-soft mb-2 block">
                    Promo / Gift Code
                  </label>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-faint" />
                      <input
                        type="text"
                        value={promoCode}
                        onChange={(e) => { setPromoCode(e.target.value); setPromoError(""); }}
                        placeholder="Enter code"
                        className="w-full pl-9 pr-3 py-2.5 text-xs border border-line focus:outline-none focus:border-aqua-400 transition-colors uppercase tracking-wider font-bold"
                      />
                      {promoApplied && (
                        <button onClick={removePromo} className="absolute right-3 top-1/2 -translate-y-1/2 text-faint hover:text-soft">
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-[0.14em] bg-brand-600 rounded-full text-white hover:bg-brand-700 disabled:bg-faint disabled:cursor-not-allowed transition-colors"
                    >
                      Apply
                    </button>
                  </div>
                ...
                <div className="space-y-3 pt-2 border-t border-line">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-soft font-medium">Subtotal</span>
                    <span className="font-bold text-soft">{formatPrice(subtotal)}</span>
                  </div>
                  {promoApplied && (
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-soft font-medium">Shipping</span>
                    <span className="font-bold text-soft">
                      {shipping === 0 ? "FREE" : formatPrice(shipping)}
                    </span>
                  </div>
                  {shipping > 0 && shippingFreeThreshold > 0 && (
                    <div className="bg-amber-50 text-amber-600 p-3 rounded-lg text-xs font-bold flex items-center justify-center gap-2">
                      <Truck className="w-4 h-4" />
                      Add {formatPrice(shippingFreeThreshold - (subtotal - discount))} more for free shipping
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-end border-t border-line pt-4">
                  <span className="text-base font-extrabold text-foreground uppercase tracking-[0.14em]">Total</span>
                ...
                <button
                  ...
                  className="w-full bg-brand-600 rounded-full text-white py-4 text-xs font-extrabold tracking-[0.14em] uppercase hover:bg-brand-700 active:scale-[0.99] transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-900/25"
                >
                ...
                <div className="grid grid-cols-3 gap-3 pt-2 border-t border-line">
                  {[
                    ...
                    <div key={i} className="flex flex-col items-center gap-1 text-faint">
                      {b.icon}
