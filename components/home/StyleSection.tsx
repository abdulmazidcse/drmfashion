...
// Shown only before an admin has ever configured the section (the
// home_style_sections Setting row is missing). Once it is saved — even with an
// empty category list — the configured data wins and these never appear again.
const FALLBACK: Record<Gender, Tile[]> = {
  men: [
...
export default function StyleSection({ title, highlight, men, women }: StyleSectionProps) {
  const configured = (men?.length ?? 0) > 0 || (women?.length ?? 0) > 0;
  const tiles: Record<Gender, Tile[]> = configured
    ? { men: men ?? [], women: women ?? [] }
    : FALLBACK;

  // Open on whichever tab actually has tiles, so a section configured for one
  ...
  return (
    <section className="w-full py-[15px] md:py-5">
      ...
      {/* Fixed column count — see the note in TrendingCategories. Deriving it
          from the tile count blew a lone tile up to half the screen. */}
