  ...
  return (
    <article className="group flex flex-col">
      <Link
        ...
        className="relative block aspect-[16/10] w-full overflow-hidden bg-cream"
      >
        {post.coverImage ? (
          <Image
            src={post.coverImage}
        ...
        {post.category && (
          <span className="absolute left-4 top-4 bg-white/95 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-foreground">
            {post.category.name}
          </span>
        )}
      </Link>
      ...
      <div className="flex flex-1 flex-col pt-5">
        ...
        <h3 className="mt-3 text-lg font-bold leading-snug tracking-tight text-foreground md:text-xl">
          <Link href={`/journal/${post.slug}`} className="transition-colors hover:text-soft">
            {post.title}
          </Link>
        </h3>

        {post.excerpt && (
          <p className="mt-2.5 line-clamp-3 text-sm leading-relaxed text-soft">{post.excerpt}</p>
        )}

        <Link
          href={`/journal/${post.slug}`}
          className="mt-4 inline-flex items-center gap-1.5 self-start border-b border-brand-600 pb-0.5 text-[11px] font-bold uppercase tracking-[0.15em] text-foreground transition-colors hover:border-brand-300 hover:text-soft"
        >
          Read more
